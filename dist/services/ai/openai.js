...
const callOpenAI = async (systemContent, userContent, isJsonMode = false) => {
...
Pick a prompt that gives the user the best opportunity to stand out on dating apps like Hinge. Return ONLY the chosen prompt.`;
        const chosenPrompt = await callOpenAI(choosePromptSystem, JSON.stringify(userProfile));
...
- Dating app: Hinge
- Desired tones: ${userProfile.tones.join(", ")}
- User interests: ${userProfile.interests.join(", ")}, ${userProfile.specificLove}
...
- Incorporate at least one of these personal details or interests: ${userProfile.interests.join(", ")}, or ${userProfile.specificLove}
